...
void GeocacheSensor::buttonProgram_Click(System::Object^  sender, System::EventArgs^  e)
{
	...
	if (this->checkBoxPIN->Checked)
	{
      ...
      {
         if(!UInt32::TryParse(this->textBoxPIN->Text, (unsigned int)GeocacheData->ulPIN))
            System::Windows::Forms::MessageBox::Show(this, "PIN could not be parsed as an unsigned 4 byte number");
      }
		else
	...
	if (this->checkBoxLoggedVisits->Checked)
	{
		...
		//if (tmLastVisit.tm_isdst > 0)  //Removed. We are calculating an absolute difference in UTC time, which is not impacted by daylight savings
...
#pragma warning (disable : 4996)
		...
		//Additional correction for daylight savings not required
...
/**************************************************************************
 * GeocacheSensor::textBoxID_TextChanged
 * 
 * Ensures ID uses characters in the valid ASCII 6 bit range.
 * We use the text changed event so we have the actual characters and don't have to mess around with key codes.
 *
 * returns: N/A
 *
 **************************************************************************/
void GeocacheSensor::textBoxID_TextChanged(System::Object^  sender, System::EventArgs^  e)
{
   System::Text::StringBuilder^ sb = gcnew System::Text::StringBuilder();
   for(int i=0; i < textBoxID->Text->Length; ++i)
   {
      if(textBoxID->Text[i] >= 32 && textBoxID->Text[i] <= 95)
         sb->Append(textBoxID->Text[i]);
   }

   textBoxID->Text = sb->ToString();
}
